...
try {
    ...
    $upcomingStmt = $pdo->prepare("SELECT event_id AS id, title, type, event_date, location FROM EventDetails WHERE event_date >= ? ORDER BY event_date ASC");
    $upcomingStmt->execute([$today]);
    $upcomingEvents = $upcomingStmt->fetchAll(PDO::FETCH_ASSOC);

    // Fetch previous events (event_date < today)
    $previousStmt = $pdo->prepare("SELECT event_id AS id, title, type, event_date, location FROM EventDetails WHERE event_date < ? ORDER BY event_date DESC");
